...
$.extend( AutoFill.prototype, {
	...
	_attach: function ( node )
	{
		...
		if ( ! this.dom.offsetParent ) {
			// We attach to the tables offset parent
	...
	_editor: function ( cells )
	{
		...
		// Build the object structure for Editors multi-row editing
		var idValues = {};
		var nodes = [];
	...
	_getPosition: function ( node, targetParent )
	{
		...
		do {
			...
			// Emergency fall back. Shouldnt happen, but just in case!
	...
	_mousedown: function ( e )
	{
		...
		// Cache scrolling information so mouse move doesnt need to read.
	...
	_mouseup: function ( e )
	{
		...
		// Havent selected multiple cells, so nothing to do
		...
		// Cant use Array.prototype.map as IE8 doesnt support it
		// Cant use $.map as jQuery flattens 2D arrays
	...
	_shiftScroll: function ( e )
	{
		...
		// DataTables scrolling calculations - based on the tables position in
		...
		// this information to the interval. Cant use local variables as they
		// wouldnt be the ones that are used by an already existing interval!
		if ( windowVert || windowHoriz || dtVert || dtHoriz ) {
			...
			// Dont need to scroll - remove any existing timer
			clearInterval( this.s.scrollInterval );
			this.s.scrollInterval = null;
		}
		...
		if ( ! this.s.scrollInterval && runInterval ) {
			this.s.scrollInterval = setInterval( function () {
				// Dont need to worry about setting scroll <0 or beyond the
