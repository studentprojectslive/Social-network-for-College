...
<html>
    ...
    protected function _drawImage($src, $subLi=false)
    {
        ...
        if ($float==left) {
            ...
            // save the current max. We dont save the X because it is not the real max of the line
            $this->_maxY = max($this->_maxY, $y+$h);
    ...
    protected function _tag_open_WRITE($param)
    {
        ...
        while ($x+$w>$right && $x<$right+$space && count($words)) {
            ...
            if (count($words)) {
                ...
                // if we dont add any word, and if the first word is empty => useless space to skip
                if (!$add && $words[0][0]===) {
                    array_shift($words);
                }
            ...
            for ($k=0; $k<count($path);true) {
                ...
                switch($lastAction)
                {
                    ...
                    case a:
                        ...
                        $action[] = 1.*$path[$k+2];                                                        // angle de deviation de laxe X
